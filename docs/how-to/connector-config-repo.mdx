...
This guide covers the full setup and day-to-day use of a dedicated **connector-config repository** — a separate git repo that holds your connector YAML files, tests, and evals, keeping them independent from the `ingest-relay` runtime repo.

## Why a Separate Repo?

| Runtime repo (`ingest-relay`) | Connector-config repo (`my-connectors`) |
|---|---|
...
Your connector-config repo needs `ingest-relay` installed to run the validation scripts and tests. Add it to `pyproject.toml`:
...
```bash
ingest-relay run --connector C:/dev/tools/ingest-relay-connectors/connectors/hr-employees.yaml
```
...
for f in /path/to/my-connectors/connectors/*.yaml; do
  ingest-relay run --connector "$f"
done
```

```powershell
# PowerShell (Windows)
Get-ChildItem C:\dev\tools\ingest-relay-connectors\connectors\*.yaml | ForEach-Object {
    ingest-relay run --connector $_.FullName
}
```

### Pointing Ops UI and Studio at your connectors

`CONNECTORS_DIR` controls which directory the API server (`ingest-relay serve`) scans for connectors to display in the Ops UI and Studio. Set it before starting the server:
...
```
CONNECTORS_DIR=C:/dev/tools/ingest-relay-connectors/connectors
```
...
For `file_pull` connectors, `source.path` is resolved relative to the **working directory of the process at the time you run `ingest-relay`** — not relative to the connector YAML file itself.

Example: if you run `ingest-relay` from `C:\dev\tools\ingest-relay` and your connector YAML contains:
...
The resolved path will be `C:\dev\tools\ingest-relay\runtime\sources\signavio`, regardless of where the YAML file lives.
