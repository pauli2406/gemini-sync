...
The `--connector` flag accepts any path to a single YAML file — the connector does not need to be inside the runtime repo. See [Connector Config Repository](/docs/how-to/connector-config-repo) for the full external repo setup guide.
