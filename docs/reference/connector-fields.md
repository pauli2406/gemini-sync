...
| `spec.output` | `object` | Yes | - | - | `sql_pull`, `rest_pull`, `rest_push`, `file_pull` | Artifact publishing destination settings. | `{bucket: gs://company-gemini-sync, prefix: hr-employees, format: ndjson, publishLatestAlias: false}` | - |
